...
table(cross_section_lines$group)
csl_end <- cross_section_lines[cross_section_lines$group == "end", ]
csl_start <- cross_section_lines[cross_section_lines$group == "start", ]
# mapview(csl_start) + mapview(csl_end)


buff <- clean_reach_buffer(center_line = center_line,
                           buffer_width = 100,
                           cross_section_lines = csl_end,
                           us_distance_colname = "us_distance_m",
                           epsg = 26910)
...
plot(sf::st_geometry(cross_section_lines[cross_section_lines$l_id == 1, ]),
     add = TRUE, col = "blue")
plot(sf::st_geometry(buff[buff$l_id == 1, ]), add = TRUE, col = "grey")
plot(sf::st_geometry(cross_section_lines[csl_end$l_id == 1, ]),
     add = TRUE, col = "blue")

plot(buff['l_id'], border = NA)

# mapview(buff) + mapview(csl_end)
