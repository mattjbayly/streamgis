test_that("test tree canopy", {
  ...
  dsm <- system.file("extdata", "./ndss/dsm_26910.tif", package = "streamgis")
  tree_raster_path <- system.file("extdata", "./ndss/tree_height.tif", package = "streamgis")
  dtm <- system.file("extdata", "./ndss/dtm_26910.tif", package = "streamgis")

  spcoa <- sample_profiles_and_canopy_angle(
    csl = csl,
    raster_path_tree_height = dsm,
    raster_path_terrain = dtm,
    step_m = 1,
    center_distance = (100 / 2),
    set_vertical_offset = 0.5
  )
  ...
  if (FALSE) {
    ...
    # read the raster
    treeh <- system.file("extdata", "./ndss/tree_height.tif", package = "streamgis")
    r <- terra::rast(treeh)

    # tree_height.tif
    # mask values < 0.5 to NA (so they render as "clear")
    r_mask <- terra::ifel(r < 0.5, NA, r)
    r_mask <- terra::ifel(r > 50, NA, r)
    ...
    for (i in 1:length(u_profile)) {
      ...
      extents_overlap <- function(e1, e2) {
        !(e1$xmax <= e2$xmin || e1$xmin >= e2$xmax ||
            e1$ymax <= e2$ymin || e1$ymin >= e2$ymax)
      }

      e_r   <- terra::ext(rM)
      over  <- extents_overlap(e_r, ext_pad)
      ...
      # shade area under the dtm
      x <- this_set$dist_m
      y <- this_set$dtm_26910
      baseline <- 0
      ...
      if (length(x) > 1) {
        polygon(
          c(x, rev(x)),
          c(rep(baseline, length(x)), rev(y)),
          col = adjustcolor("brown", alpha.f = 0.6),
          border = NA
        )
      }
